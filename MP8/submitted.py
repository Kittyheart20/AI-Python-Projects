...
def utility_partials(R, x):
    ...
    partial[0] = dp0 @ R[0,:,:] @ p1
    partial[1] = p0 @ R[1,:,:] @ dp1
...
def episodic_game_gradient_ascent(init, rewards, nsteps, learningrate):
    ...
    for t in range(nsteps - 1):
        ...
        utilities[t, 0] = p0 @ rewards[0, :, :] @ p1
        utilities[t, 1] = p0 @ rewards[1, :, :] @ p1

        l = utility_partials(rewards, logits[t, :])
        
        logits[t + 1, 0] = logits[t, 0] + learningrate * l[0]
        logits[t + 1, 1] = logits[t, 1] + learningrate * l[1]

    #raise RuntimeError("You need to write this!")   
    print("logits: ", logits, " utilities: ", utilities)         
...
def utility_hessian(R, x):
    ...
    hessian[0, 0] = hp0 @ R[0,:,:] @ p1
    hessian[1, 1] = p0 @ R[1,:,:] @ hp1
    hessian[0, 1] = dp0 @ R[0,:,:] @ dp1
    hessian[1, 0] = dp1 @ R[1,:,:] @ dp0
...
def episodic_game_corrected_ascent(init, rewards, nsteps, learningrate):
    ...
    for t in range(nsteps - 1):
        ...
        utilities[t, 0] = p0 @ rewards[0, :, :] @ p1
        utilities[t, 1] = p1 @ rewards[1, :, :] @ p0

        partials = utility_partials(rewards, logits[t, :])
        hessian = utility_hessian(rewards, logits[t, :])

        l = learningrate * (np.eye(2)  + symplectic_correction(partials, hessian)) @ partials

        logits[t + 1, 0] = logits[t, 0] + l[0]
        logits[t + 1, 1] = logits[t, 1] + l[1]
...
'''
sequential_strategy = 0.2*np.ones((2,2)) # Default: always play uniformly at random
