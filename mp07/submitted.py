...
def minimax(board, side, flags, depth):
    ...
    for move in moves:
        key = encode(move[0], move[1], move[2])
        newside, newboard, newflags = makeMove(side, board, move[0], move[1], flags, move[2])

        moveList, moveTree, value = minimax(newboard, newside, newflags, depth - 1)
        
        mtree[key] = moveTree  # Store the subtree for this move
        

        if (side and value < bestVal) or (not side and value > bestVal):
            bestVal = value
            best_move_list = [move] + moveList

    return best_move_list, mtree, bestVal
...
def alphabeta(board, side, flags, depth, alpha=-math.inf, beta=math.inf):
    ...
    for move in moves:
        ...
        moveList, moveTree, value = alphabeta(newboard, newside, newflags, depth - 1, alpha, beta)
